...
dscAdapter* DSC;
#if defined(ESP32)
SemaphoreHandle_t xProcessEvent;
#endif
...
void pcid_callback(char qualifier,const String event,const String partition,const String zone_user) {//recibe los eventos del panel y los envía
    #if defined(ESP32)
    if(xSemaphoreTake( xProcessEvent, portMAX_DELAY ) == pdTRUE){
    #endif
            ...
            if(flagR1){
                if(event == readyID){ //Que mande nomás el cambio de estado de ready/not ready de la partición
                    if( partition.charAt(1) == '1'){
                        if(zone_user == zoneuPrevPart1 && !newR1ReceptionPart1){
                            #if defined(ESP32)
                            xSemaphoreGive( xProcessEvent );
                            #endif
                            return;
                        }
                        ...
                        //Esto se hace porque a veces el panel manda 3 listos seguidos, cuando con mandar uno solo, es suficiente
                            ...
                            zoneuPrevPart1 = zone_user;
                        }
                    }
                    if( partition.charAt(1) == '2'){
                        if(zone_user == zoneuPrevPart2 && !newR1ReceptionPart2){
                            #if defined(ESP32)
                            xSemaphoreGive( xProcessEvent );
                            #endif
                            return;
        ...
        String strMsg=qualifier + event + partition + zone_user;        
        if(qualifier!=charS){
            EventP e; 
            strncpy(e.msg,strMsg.c_str(),SIZE_MSG);
            queue->Push(e);
...
byte pcid_transmitComand(){
    ...
    if(comando == gprsActive){
        Serial.println("Activo módulo gprs");
        digitalWrite(ResetGprs,LOW);
        return 0;
    }
    if(comando == gprsDesactive){
        Serial.println("Desactivo módulo gprs");
        digitalWrite(ResetGprs,HIGH);
        return 0;
    }
...
void pcid_callLog(String logPayload){
